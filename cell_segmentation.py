...
# Remove the PyQt5 import and use pathlib.Path instead for file checks
...
def main():
    ...
    if args.create_ctc_set:
        ...
        if not (path_datasets / 'ctc_training_set' / 'train').exists():
    ...
    if args.evaluate:
        mode.append('eval')
        if not (Path.cwd() / 'EvaluationSoftware').exists():
        ...
        for label_type in methods:
            ...
            for model in models:
                ...
                for cell_type in cell_types:
                    ...
                    if (path_results / model.stem / cell_type).exists():
                        if not (not list((path_results / model.stem / cell_type).glob('mask*'))):
                            ...
                            if (path_results / "metrics.json").exists():
    ...
    if 'plot' in mode:
        ...
        if not (path_results / 'metrics.json').exists():
